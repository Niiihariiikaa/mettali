...
function Field({ label, name, required, type = "text", textarea, placeholder }) {
  ...
  return (
    <label style={{ display: "block", position: "relative" }}>
      ...
      {textarea ? (
        <textarea name={name} rows={4} required={required} placeholder={placeholder}
...
/* ══════════════════════════════════════════════════════
   Shared <style> block for the cz-* animation classes used
   by CustomisationIntro / CustomisationForm / the hero.
   Exported so pages that reuse only a subset of the sections
   (e.g. the customise/studio page) can still pull in the CSS.
   ══════════════════════════════════════════════════════ */
export function CustomisationStyles() {
  return (
    <style>{`
...
/* ══════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════ */
export function CustomisationIntro({ light = false }) {
  const litColor = light ? DK : RL;
      ...
      <div style={{ marginTop: 80 }}>
        <Reveal>
          ...
          <h2 style={{
            ...
            textTransform: "uppercase", color: headingColor, lineHeight: 1.15,
  ...
  return (
    <section style={{ background: TP, marginTop: -1, padding: "56px 24px" }}>
      <div style={{
...
export function CustomisationForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    data.set(
      "customizations",
      Object.entries(customizations).filter(([, v]) => v).map(([k]) => k).join(", ")
    );

    try {
      const res = await fetch("/api/customisation-request", { method: "POST", body: data });
      if (!res.ok) {
      ...
      {submitted ? (
        ...
        <form onSubmit={handleSubmit}>
          ...
          {requestType === "customize" ? (
            <Reveal>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 40, marginBottom: 72 }}>
                <Field label="Piece You Wish to Customize (Name)" name="pieceName" required />
                <Field label="Product Link (Optional)" name="productLink" placeholder="https://" />
                ...
                <Field label="Describe Your Requirements" name="requirements" required textarea />
              </div>
            </Reveal>
          ) : (
            <Reveal>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 40, marginBottom: 72 }}>
                <Field label="Describe the Piece You Would Like Us to Create" name="requirements" required textarea />
                ...
                <div>
                  ...
                  <label style={{
                    ...
                    <input name="files" type="file" multiple accept="image/*,.pdf" style={{ display: "none" }} />
                  </label>
                </div>
          ...
          <Reveal>
            {error && (
              <p style={{
                fontFamily: SPACE, fontSize: 12, color: "#a33",
            ...
            <button
              ...
              style={{
                ...
                padding: "22px 0", cursor: submitting ? "wait" : "pointer",
